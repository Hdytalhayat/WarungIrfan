...
class FrontController extends Controller
{
    public function index(){
        $featured_menu  = menu::where('status', '0')->take(15)->get();
        $trending_menu  = Categories::where('popular', '1')->take(15)->get();
        return view('frontend.index' , compact('featured_menu', 'trending_menu'));
    }
    ...
    public function menuajax(){
        $menu = menu::select('name')->where('status', '0')->get();
        $data = [];
    ...
    public function search(Request $request)
    {
        ...
        if($search != ''){
            ...
            if($menu){
                return redirect('categories/'.$menu->Categories->slug.'/'.$menu->slug);
            }else{
    ...
    public function qrcode($id){
        ...
        return view('frontend.qrcode', compact('menu'));
    }
}
